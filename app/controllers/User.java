...
public class User extends Controller {
	...
	public static Result resetPassword() {
		...
		if(user!=null){
			flash("success","Инструкции по смене пароля отправлены на адрес <b>"+email+"</b>");
			Emailing.send("Восстановление пароля от Қора.kz", new String[]{user.userName+" <"+user.email+">"}, recoverPassword.render(user).body());
		}
		else{
			flash("error","Личный кабинет с указанным логином не зарегистрирован, хотите <a href='#' data-toggle='modal' data-target='#myModal2'>зарегистрироваться</a>?");
		}
	...
	public static Result changePassword(String code) {
		AuthorisedUser user = AuthorisedUser.findByEmail(play.libs.Crypto.decryptAES(code));
		DynamicForm requestData = form().bindFromRequest();
		...
		if(!newPassoword.equals(repeatPassoword)){
			...
			user.update();
			flash("success","Пароль успешно изменен! Хотите <a href='#' data-toggle='modal' data-target='#myModal'>войти</a>? ");
		}
		return redirect(request().getHeader("referer"));
	}
