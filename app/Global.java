...
public class Global extends GlobalSettings {
	...
	public void onStart(Application application) {
		...
		if (Category.find.findRowCount() == 0) {
			for (String name : Arrays.asList("Арабский скакун", "Император",
					"Адайская порода", "Казахская порода", "Кушумская порода",
					"Мугалжарская порода", "Костанайская порода",
					"Ахалтекинская порода", "Джэбе (Жабы)", "Калмыцкая порода",
					"Кабардинская порода", "Карачаевская порода",
					"Каспийская порода", "Русская верховая", "Русский рысак",
					"Русский тяжеловоз", "Донская порода", "Башкирская порода",
					"Белорусская упряжная", "Азербайджанская порода"
			...
			for (String name : Arrays.asList("Монгольская порода",
					"Порода Арвана (аруана)", "Казахская порода",
					"Калмыцкая порода")) {
				Category category = new Category();
				category.section = Section.find.where().eq("name", "Верблюд")
						.findUnique();
				category.name = name;
				category.save();
			}

			for (String name : Arrays.asList("Казахская белоголовая порода")) {
				Category category = new Category();
				category.section = Section.find.where().eq("name", "Корова")
						.findUnique();
				category.name = name;
				category.save();
			}

			for (String name : Arrays.asList("Каракульская порода",
					"Апшеронская овца", "Североказахский меринос",
					"Романовская порода", "Курдючные породы",
					"Армянская полугрубошерстная овца",
					"Сокольская порода овец", "Алтайская порода овец",
					"Грубошёрстная померанская порода овец",
					"Кавказская порода овец", "Казахская порода овец")) {
				Category category = new Category();
				category.section = Section.find.where().eq("name", "Овцы/Козы")
						.findUnique();
				category.name = name;
				category.save();
			}

			for (String name : Arrays.asList("Сельхозтехника",
					"Сельхозпродукты")) {
				Category category = new Category();
		...
		if (Region.find.findRowCount() == 0) {
			for (String name : Arrays.asList("Атырауская область",
					"Алматинская область", "Акмолинская область",
					"Актюбинская область", "Восточно-Казахстанская область",
					...
					"Повлодарская область", "Северо-Казахстанская область",
